...
//		Msg("new frame, %d", Device.dwFrame);
		// initial setup
...
void	CRenderTarget::phase_accumulator_reflected ()
{
	// Targets
	if (dwAccumulatorClearMark/*Refl*/==Device.dwFrame)	{
		// normal operation - setup
		/*if (RImplementation.o.fp16_blend)	*/u_setrt	(rt_Accumulator/*_reflected*/,		NULL,NULL,HW.pBaseZB);
		//else								u_setrt	(rt_Accumulator_temp,	NULL,NULL,HW.pBaseZB);
	} else {
		// initial setup
		dwAccumulatorClearMark/*Refl*/				= Device.dwFrame;

		// clear
		u_setrt								(rt_Accumulator/*_reflected*/,		NULL,NULL,HW.pBaseZB);
		dwLightMarkerID						= 5;					// start from 5, increment in 2 units
		u32		clr4clear					= color_rgba(0,0,0,0);	// 0x00
		CHK_DX	(HW.pDevice->Clear			( 0L, NULL, D3DCLEAR_TARGET, clr4clear, 1.0f, 0L));
