...
//	if (IsMainMenuActive())		return;
	...
	*/
	...
	{
		Fmatrix			m_ViewToLightView;
		Fmatrix			xf_world;		xf_world.invert	(Device.mView);
		m_ViewToLightView.mul			(L->X.S.view, xf_world);
		// Select shader
		u32		_id					= 0;
		if (L->flags.bShadow)		{
		...
		if (ps_r2_test_flags.test(R2FLAG_PCSS))
		{
			...
			RCache.set_c				("pcss_params",		L->smap_near, L->range+EPS_S, L->smap_near/RImplementation.o.smapsize, L->smap_near/RImplementation.o.smapsize);
		}
//		Msg("point light: ptr %d frame %d lsf_params: %f,%f,%f", (u32)L, Device.dwFrame, L->lsf_speed, L->lsf_amount, L->lsf_smap_jitter);
