...
//	{
		...
		if (SE_SUN_NEAR==sub_phase)
			sun_near_shadow_xform.set(m_shadow);
		else
			sun_far_shadow_xform.set(m_shadow);
		...
		Fmatrix				m_clouds_shadow;
		{
			...
			m_clouds_shadow.mulA_44		(m_xform)					;
			if (SE_SUN_NEAR==sub_phase)
				sun_clouds_shadow_xform.set(m_clouds_shadow);
		}
...
//		Msg("LDynamic_color: %f,%f,%f,%f", L_clr.x,L_clr.y,L_clr.z,L_spec);
...
//	}
	...
	// KD 
	// add sunshafts check here
	accum_direct_volumetric(sub_phase, m_shadow);
}
