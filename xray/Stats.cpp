...
void CStats::Show() 
{
	// Stop timers
	{
		...
		RenderDUMP_DOF.FrameEnd				();
		RenderDUMP_Sunshafts.FrameEnd				();
		RenderDUMP_SSAO.FrameEnd				();
		RenderDUMP_AA.FrameEnd				();
...
/*		CSound_stats				snd_stat;
		::Sound->statistic			(&snd_stat,0);			TEMP - KD*/
		...
		if (!ps_service_flags.test(S_EXTENDED_GRAPH_STATS)) {
...
//		F.OutSet	(640,0);
	...
	#define PPP(a) (100.f*float(a)/float(RenderTOTAL.result))
			...
			F.OutNext	("  S_render:  %2.2fms, %d",RenderDUMP_Srender.result,RenderDUMP_Srender.count);
			F.OutSkip	();
			F.OutNext	("  DOF:       %2.2fms",RenderDUMP_DOF.result);	
			F.OutNext	("  Shafts:    %2.2fms",RenderDUMP_Sunshafts.result);
			F.OutNext	("  SSAO:	   %2.2fms",RenderDUMP_SSAO.result);
			F.OutNext	("  AA:		   %2.2fms",RenderDUMP_AA.result);
			F.OutSkip	();
		}
...
/*		F.SetHeightI						(f_base_size);
		F.OutSet						(400,0);
		g_pGamePersistent->Statistics	(&F);			TEMP - KD*/

		//////////////////////////////////////////////////////////////////////////
		// process PURE STATS
/*		F.SetHeightI						(f_base_size);
		seqStats.Process				(rp_Stats);
		pFont->OnRender					();				TEMP - KD*/
	};
...
/*	if( psDeviceFlags.test(rsStatistic) || psDeviceFlags.test(rsCameraPos) ){
...

	{
		...
		RenderDUMP_DOF.FrameStart				();
		RenderDUMP_Sunshafts.FrameStart				();
		RenderDUMP_SSAO.FrameStart				();
		RenderDUMP_AA.FrameStart				();
