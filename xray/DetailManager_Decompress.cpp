...
void		CDetailManager::cache_Decompress(Slot* S)
{
	...
	float	dist_sq = 1.f;
/*	if (ps_r2_ls_flags.test(R2FLAG_DETAILS_LOW))
	{
		Fvector dist		= Device.vCameraPosition;
		dist.sub(D.vis.sphere.P);
		dist_sq				= dist.magnitude();
		dist_sq				= (dist_sq>40.f)?(1.f + (dist_sq-40.f)/(dm_current_size-40.f)):1.f;
	}*/
	float		density		= ps_r__Detail_density*dist_sq;
