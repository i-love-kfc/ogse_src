...
float		ps_r2_rmap_tsm_projection	= 0.3f;			// 0.18f
float		ps_r2_rmap_tsm_bias			= -0.01f;			// 
float		ps_r2_rmap_near				= 20.f;				// 12.0f
float		ps_r2_rmap_near_border		= 0.99f;			// 1.0f
float		ps_r2_rmap_depth_far_scale	= 1.00000f;			// 1.00001f
float		ps_r2_rmap_depth_far_bias	= 0.00000f;			// -0.0000f
float		ps_r2_rmap_depth_near_scale	= 1.f; // was 1.00001f;			// 1.00001f
float		ps_r2_rmap_depth_near_bias	= 0.f; //was -0.00004f;		// -0.00005f
float		ps_r2_sharpening			= 0.f;
float		ps_r2_ndr					= 0.f;
float		ps_r2_ao_intensity			= 1.f;
int			is_lods_limited				= 0;
...
class CCC_SSADiscard		: public CCC_Float
{
public:
	void	apply	()	{
		ps_r__ssaDISCARD = _sqr(*value);
	}
	CCC_SSADiscard(LPCSTR N, float* V, float _min=0, float _max=1.0) : CCC_Float(N, V, _min, _max)		{ };
	virtual void Execute	(LPCSTR args)
	{
		CCC_Float::Execute	(args);
		apply();
...
//-----------------------------------------------------------------------
class CCC_RWS : public IConsole_Command
{
public:
	CCC_RWS(LPCSTR N) : IConsole_Command(N) { bEmptyArgsHandled = TRUE; };
	virtual void Execute(LPCSTR args) {
		w_states.set(0,0,0,1);
	}
};
//-----------------------------------------------------------------------
class CCC_QS : public IConsole_Command
{
public:
	CCC_QS(LPCSTR N) : IConsole_Command(N) { bEmptyArgsHandled = TRUE; };
	virtual void Execute(LPCSTR args) {
		save_game_quick();
	}
};
//-----------------------------------------------------------------------
class CCC_Textures : public IConsole_Command
{
public:
	CCC_Textures(LPCSTR N) : IConsole_Command(N) { bEmptyArgsHandled = TRUE; };
	virtual void Execute(LPCSTR args) {
		Msg("TEXTURES USING");
		CResourceManager::map_Texture texs = Device.Resources->m_textures;		
		for(CResourceManager::map_TextureIt it = texs.begin(), end = texs.end(); it != end; ++it )
		{
			if (it->first)
				Msg("%s: %d", it->first, it->second->flags.MemoryUsage);
		}
		if (RImplementation.Models)
		{
			u32 size = RImplementation.Models->Models.size();
			Msg("MODELS POOL LOADED: %d", size);
			for (u32 i = 0; i < size; ++i)
			{
				Msg("model: %s, refs: %d", RImplementation.Models->Models[i].name.c_str(), RImplementation.Models->Models[i].refs);
			}
			size = RImplementation.Models->Registry.size();
			Msg("MODELS REGISTRY LOADED: %d", size);
			for (CModelPool::REGISTRY_IT it = RImplementation.Models->Registry.begin(), end = RImplementation.Models->Registry.end(); it != end; ++it)
			{
				if (it->first)
					Msg("registry: %s", it->second.c_str());
			}
			size = RImplementation.Models->Pool.size();
			Msg("MODELS POOL LOADED: %d", size);
			for (CModelPool::POOL_IT it = RImplementation.Models->Pool.begin(), end = RImplementation.Models->Pool.end(); it != end; ++it)
			{
				if (it->second)
					Msg("pool: %s", it->first.c_str());
			}
		} else {
...
//	CMD4(CCC_Float,		"r__wallmark_ttl",		ps_r__WallmarkTTL,			1.0f,	5.f*60.f);
	...
	CMD4(CCC_Float,		"r__geometry_lod",		&ps_r__LOD,					0.1f,	5.0f		);
	CMD4(CCC_Float,		"r__detail_density",	&ps_current_detail_density,		.06f,	0.6f	);
...
//	CMD3(CCC_Mask,		"r2_allow_r1_lights",	&ps_r2_ls_flags,			R2FLAG_R1LIGHTS	);
	...
	//- Mad Max
	CMD4(CCC_Float,		"r2_gloss_factor",		&ps_r2_gloss_factor,		.1f,	10.f	);
	//- Mad Max
	...
	CMD4(CCC_Float,		"r2_sun_near",			&ps_r2_sun_near,			1.f,	60.f	);
	...
	CMD4(CCC_Float,		"r2_sun_lumscale",		&ps_r2_sun_lumscale,		0.0,    2.0/*-1.0,	+3.0*/	);
...
//	CMD3(CCC_Mask,		"r2_rain_map",				&ps_r2_test_flags,		R2FLAG_RAIN_MAP);
	...
	CMD4(CCC_Float,		"r2_ao_intensity",			&ps_r2_ao_intensity,		0.2,	1.0		);
...
//	CMD1(CCC_Textures, "dump_textures");
...
void	xrRender_apply_tf		()
{
	...
	Console->Execute	("r2_tf_mipbias"); // R2 only
}
